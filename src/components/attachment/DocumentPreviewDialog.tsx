...
export function DocumentPreviewDialog({ attachment, onClose }: DocumentPreviewDialogProps) {
  ...
  const open = attachment !== null && attachment.isDocument
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-hidden bg-[var(--color-surface)]">
          {attachment.type === 'csv' ? (
            <CsvPreview attachment={attachment} />
          ) : attachment.type === 'xls' || attachment.type === 'xlsx' ? (
            <ExcelPreview attachment={attachment} />
          ) : (
